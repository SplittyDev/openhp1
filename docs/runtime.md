...
UE1 `GetAxes` and `vector(Rotator)` use the coordinate-frame axes returned by
`FRotation`, whose yaw direction is opposite the local-vector `<<` transform.
Do not implement both operations with the same matrix.
